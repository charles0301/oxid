...
{
    var novalnet_sepa_holder = novalnet_sepa_country = novalnet_sepa_account_no = novalnet_sepa_bank_code = novalnet_sepa_uniqueid = novalnet_vendor = novalnet_auth_code = '';
    $("#novalnet_sepa_loader").show();
    if($('#novalnet_sepa_vendor_id').length) {
        novalnet_vendor = $('#novalnet_sepa_vendor_id').val();
    }
    if($('#novalnet_sepa_vendor_authcode').length) {
        novalnet_auth_code = $('#novalnet_sepa_vendor_authcode').val();
    }
    if($('#novalnet_sepa_uniqueid').length) {
        novalnet_sepa_uniqueid = $('#novalnet_sepa_uniqueid').val();
    }
    if($('#novalnet_sepa_holder').length) {
        novalnet_sepa_holder = $.trim($('#novalnet_sepa_holder').val());
    }
    if($('#novalnet_sepa_country').length) {
        novalnet_sepa_country = $('#novalnet_sepa_country').val();
    }
    if($('#novalnet_sepa_acc_no').length) {
        novalnet_sepa_account_no = removeSpecialCharactersSepa($('#novalnet_sepa_acc_no').val());
    }
    if($('#novalnet_sepa_bank_code').length) {
        novalnet_sepa_bank_code = removeSpecialCharactersSepa($('#novalnet_sepa_bank_code').val());
    }
    if($('#novalnet_remote_ip').length) {
        novalnet_remote_ip = $('#novalnet_remote_ip').val();
    }
    if(novalnet_vendor == '' || novalnet_auth_code == '') {
        $('#novalnet_sepa_mandate_confirm').attr('checked', false);
        $("#novalnet_sepa_loader").hide();
    ...
    if(novalnet_sepa_holder == '' || novalnet_sepa_country == '' || novalnet_sepa_account_no == '' || novalnet_sepa_bank_code == '' || novalnet_sepa_uniqueid == '') {
        $('#novalnet_sepa_mandate_confirm').attr('checked', false);
        $("#novalnet_sepa_loader").hide();
        alert($('#novalnet_sepa_invalid_message').val());
        return false;
...
{
    var novalnet_sepa_country = novalnet_sepa_holder = novalnet_sepa_account_no = novalnet_sepa_bank_code = novalnet_sepa_uniqueid = novalnet_vendor = novalnet_auth_code = novalnet_sepa_iban = novalnet_sepa_bic = '';
    $("#novalnet_sepa_loader").show();
    if($('#novalnet_sepa_vendor_id').length) {
        novalnet_vendor = $('#novalnet_sepa_vendor_id').val();
    }
...
{
    var novalnet_vendor = novalnet_auth_code = novalnet_sepa_uniqueid = novalnet_sepa_hash = '';
    $("#novalnet_sepa_loader").show();
    if($('#novalnet_sepa_vendor_id').length) {
        novalnet_vendor = $('#novalnet_sepa_vendor_id').val();
    }
    ...
    if(novalnet_vendor == '' || novalnet_auth_code == '' || novalnet_sepa_uniqueid == '') {
        $("#novalnet_sepa_loader").hide();
        return false;
    }
