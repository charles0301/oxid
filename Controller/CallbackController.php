...
class CallbackController extends FrontendController
{
    ...
    private function _processNovalnetCallback()
    {
        ...
        if ($iPaymentTypeLevel === 0) {
            if ($this->_aCaptureParams['subs_billing'] == 1 ) {
                ...
                if ($this->_aCaptureParams['status'] == '100' && in_array($this->_aCaptureParams['tid_status'], ['100',  '90', '91', '98', '99', '85'])) {
                    $sOrderComments =  'Novalnet transaction details<br>' . 'Novalnet transaction ID' . ' ' . $this->_aCaptureParams['tid'] . '<br>';
                    $sOrderComments .= !empty($this->_aCaptureParams['test_mode']) ? 'Test order' .'<br>' : '';

                    $sNovalnetComments = '<br><br>Novalnet Callback Script executed successfully for the subscription TID:' . $this->_aCaptureParams['signup_tid'] . ' with amount: ' . $dFormattedAmount . ' on ' . date('Y-m-d H:i:s') . '. Please refer PAID transaction in our Novalnet Merchant Administration with the TID: ' . $this->_aCaptureParams['tid'];
                    $sPaidUntil = !empty($this->_aCaptureParams['next_subs_cycle']) ? $this->_aCaptureParams['next_subs_cycle'] : $this->_aCaptureParams['paid_until'];
                    if (!empty($sPaidUntil)) {
                    ...
                    $sTerminationReason = !empty($this->_aCaptureParams['status_message']) ? $this->_aCaptureParams['status_message'] : $this->_aCaptureParams['termination_reason'];
                    ...
                    $sNovalnetComments = '<br><br>Novalnet callback script received. Subscription has been stopped for the TID: ' . $this->_aCaptureParams['shop_tid'] . ' on ' . date('Y-m-d H:i:s') . '.<br>Reason for Cancellation: ' . $sTerminationReason;
                ...
                if (!isset($dPaidAmount)) {
                    ...
                    $sNovalnetComments = '<br><br>Novalnet Callback Script executed successfully for the TID: ' . $this->_aCaptureParams['tid'] . ' with amount ' . $dFormattedAmount . ' on ' . date('Y-m-d H:i:s');

                    $this->_oDb->execute('UPDATE oxorder SET OXPAID = "' . date('Y-m-d H:i:s') . '", NOVALNETCOMMENTS = CONCAT(IF(NOVALNETCOMMENTS IS NULL, "", NOVALNETCOMMENTS), "' . $sNovalnetComments . '") WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"');
                    ...
                    $sNovalnetComments = '<br><br>The transaction has been canceled due to: ' . $this->_oNovalnetUtil->setNovalnetPaygateError($this->_aCaptureParams);
            ...
            $sNovalnetComments = '<br><br>Novalnet callback received. Chargeback executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' amount ' . $dFormattedAmount . ' on ' . date('Y-m-d H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            ...
            if (in_array($this->_aCaptureParams['payment_type'], ['CREDITCARD_BOOKBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU'])) {
                $sNovalnetComments = '<br><br>Novalnet callback received. Refund/Bookback executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' amount ' . $dFormattedAmount . ' on ' . date('Y-m-d H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            }
            $sUpdateSql = 'UPDATE oxorder SET NOVALNETCOMMENTS = CONCAT(IF(NOVALNETCOMMENTS IS NULL, "", NOVALNETCOMMENTS), "' . $sNovalnetComments . '") WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"';
            $this->_oDb->execute($sUpdateSql);
            $sNovalnetComments = $this->_sendMail($sNovalnetComments) . $sNovalnetComments;
            ...
            if (!isset($dPaidAmount) || $dPaidAmount < $dAmount) {
                ...
                $sNovalnetComments = '<br><br>Novalnet Callback Script executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with amount ' . $dFormattedAmount . ' on ' . date('Y-m-d H:i:s') . '. Please refer PAID transaction in our Novalnet Merchant Administration with the TID: ' . $this->_aCaptureParams['tid'];
    ...
    private function _getOrderDetails()
    {
        ...
        $sSql     = 'SELECT trans.ORDER_NO, trans.TOTAL_AMOUNT,trans.NNBASKET, trans.REFUND_AMOUNT, o.OXPAYMENTTYPE FROM novalnet_transaction_detail trans JOIN oxorder o ON o.OXORDERNR = trans.ORDER_NO where trans.tid = "' . $this->_aCaptureParams['shop_tid'] . '"';
    ...
    private function _createFollowupOrder($sOrderComments)
    {
        ...
        $sOrderComments .= (in_array($aTxndetails['OXPAYMENTTYPE'], ['novalnetinvoice','novalnetprepayment'])) ? $this->_getBankdetails().'<br><br>' : '';
    ...
    public function _insertNovalnetPreInvTable($oOrderNr, $iCnt)
    {
        ...
        $aNNPreInvDetails['DUE_DATE'] = date('d.m.Y', strtotime($this->_aCaptureParams['due_date']));
    ...
    protected function _getBankdetails()
    {
        ...
        if (!empty($this->_aCaptureParams['due_date'])) {
            $sInvoiceComments .= $oLang->translateString('NOVALNET_DUE_DATE') . date('d.m.Y', strtotime($this->_aCaptureParams['due_date']));
        }
        $sInvoiceComments .= $oLang->translateString('NOVALNET_ACCOUNT') . $this->_aCaptureParams['invoice_account_holder'];
        ...
        $sInvoiceComments .= $oLang->translateString('NOVALNET_AMOUNT') . $this->_aCaptureParams['amount'] . ' ' . $this->_aCaptureParams['currency'];
