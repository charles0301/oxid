...
class CallbackController extends FrontendController
{
        ...
        if ($iPaymentTypeLevel === 0) {
            if ($this->_aCaptureParams['subs_billing'] == 1 ) {
                ...
                if ($this->_aCaptureParams['status'] == '100' && in_array($this->_aCaptureParams['tid_status'], ['100',  '90', '91', '98', '99', '85'])) {
                    ...
                    $sNovalnetComments = 'Novalnet callback script received. Subscription has been stopped for the TID: ' . $this->_aCaptureParams['shop_tid'] . ' on ' . date('Y-m-d H:i:s') . '.<br>Reason for Cancellation: ' . $sTerminationReason;
    ...
    private function _getOrderDetails()
    {
        ...
        $sSql     = 'SELECT trans.ORDER_NO, trans.TOTAL_AMOUNT,trans.NNBASKET, trans.REFUND_AMOUNT, trans.PAYMENT_ID, o.OXPAYMENTTYPE FROM novalnet_transaction_detail trans JOIN oxorder o ON o.OXORDERNR = trans.ORDER_NO where trans.tid = "' . $this->_aCaptureParams['shop_tid'] . '"';
    ...
    private function _createFollowupOrder()
    {
        ...
        $sOrderComments .= (in_array($aTxndetails['OXPAYMENTTYPE'], ['novalnetinvoice','novalnetprepayment'])) ? $this->_getBankdetails().'<br>' : '';
    ...
    protected function _getBankdetails()
    {
        ...
        $sInvoiceComments .= $this->_oLang->translateString('NOVALNET_AMOUNT') . $this->_aCaptureParams['amount'] . ' ' . $this->_aCaptureParams['currency'];
