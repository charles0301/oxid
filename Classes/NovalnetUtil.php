...
class NovalnetUtil {
    ...
    public function setAffiliateCredentials(&$aRequest, $iCustomerNo)
    {
        ...
        if (!empty($aResult['AFF_ID']))
            $this->oSession->setVariable('dNnAffid', $aResult['AFF_ID']);

        // checks Novalnet affiliate id in session
        if ($this->oSession->getVariable('dNnAffid')) {
            $aResult = $oDb->getRow('SELECT AFF_AUTHCODE, AFF_ACCESSKEY FROM novalnet_aff_account_detail WHERE AFF_ID = "' . $this->oSession->getVariable('dNnAffid') . '"');
            if (!empty($aResult['AFF_AUTHCODE']) && !empty($aResult['AFF_ACCESSKEY'])) {
                $aRequest['vendor']    = $this->oSession->getVariable('dNnAffid');
    ...
    private function _importUserDetails(&$aRequest, $oUser)
    {
        ...
        if (!empty($oUser->oxuser__oxcompany->value)) {
            $aRequest['company'] = $oUser->oxuser__oxcompany->value;
        } else {
            $oAddress = $oUser->getSelectedAddress();
            $aRequest['company'] = $oAddress->oxaddress__oxcompany->value;
        }
