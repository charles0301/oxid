...
class NovalnetUtil {
    ...
    private function _importUserDetails(&$aRequest, $oUser)
    {
        ...
        if (!empty($oUser->oxuser__oxcompany->value))
            $aUserDetails['company'] = $oUser->oxuser__oxcompany->value;
        ...
        $aUserDetails['lang'] = strtoupper($this->oLang->getLanguageAbbr());
        $aUserDetails['country_code'] = $this->getCountryISO($oUser->oxuser__oxcountryid->value);
    ...
    private function _importOrderDetails(&$aRequest, $oBasket)
    {
        ...
        if (in_array($this->sCurrentPayment, array( 'novalnetcreditcard', 'novalnetsepa', 'novalnetinvoice', 'novalnetpaypal' )) && is_numeric($dOnHoldLimit) && $dOnHoldLimit <= $aRequest['amount'])
            $aRequest['on_hold'] = 1;
    ...
    private function _importGuaranteedPaymentParameters(&$aRequest)
    {
        if ($this->oSession->getVariable('blGuaranteeEnabled' . $this->sCurrentPayment)) {
            ...
            $aRequest['payment_type'] = $aRequest['key'] == 27 ? 'GUARANTEED_INVOICE_START' : 'GUARANTEED_DIRECT_DEBIT_SEPA';
    ...
    private function _importPaymentDetails(&$aRequest)
    {
        ...
        if ($this->sCurrentPayment == 'novalnetcreditcard') {
            ...
            if (isset($aDynValue['novalnet_cc_new_details']) && $aDynValue['novalnet_cc_new_details'] == '0') {
                ...
                if ($this->getNovalnetConfigValue('iShopTypenovalnetcreditcard') != '' && $this->getNovalnetConfigValue('blCC3DActive') == '1') {
                    $aRequest['cc_3d'] = 1;
                ...
                } elseif ($this->getNovalnetConfigValue('blCC3DActive') == '1') {
                    $aRequest['cc_3d'] = 1;
                    array_push($this->aRedirectPayments, 'novalnetcreditcard'); // checks to set credit card payment as redirect
