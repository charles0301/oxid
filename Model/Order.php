...
class Order extends Order_parent
{
    ...
    private function _logNovalnetTransaction($oBasket)
    {
       ...
       if ($this->_aNovalnetData['key'] == '6' && (isset($this->_aNovalnetData['cc_3d']) || $this->_oNovalnetUtil->getNovalnetConfigValue('blCC3DFraudActive') == '1')) {
             $bCreditcardflag = 1;
             $this->_aNovalnetData['amount'] = $this->_aNovalnetData['amount'];
        }
        if ($this->_aNovalnetData['key'] == '6' && !isset($this->_aNovalnetData['cc_3d']) && $this->_oNovalnetUtil->getNovalnetConfigValue('blCC3DFraudActive') == '0') {
             $this->_aNovalnetData['amount'] = $this->_aNovalnetData['amount']*100;
        }

         if ((in_array($this->_aNovalnetData['key'], [ '27', '37', '40', '41','59' ])) || ($this->_aNovalnetData['key'] == '34' && isset($aRequest['payment_ref'])) && empty($bCreditcardflag)) {
                $this->_aNovalnetData['amount'] = $this->_aNovalnetData['amount'] * 100;
        }
